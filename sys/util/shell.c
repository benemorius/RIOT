...
static float _read_supply_voltage(void)
{
    uint32_t sample = adc_sample(3, ADC_RES_16BIT);
    float raw_voltage = (float)sample / 65535 * ADC_REF_VOLTAGE;
    float supply_voltage = raw_voltage * 21;
    return supply_voltage;
}

static int read_supply_voltage(int argc, char **argv)
{
    (void)argc;
    (void)argv;
    float supply_voltage = _read_supply_voltage();
    printf("supply voltage: %" PRIi32 ".%03" PRIu32 "\n", (uint32_t)supply_voltage, (uint32_t)(supply_voltage * 1000) % 1000);
    return 0;
}
#endif
...
static const shell_command_t _util_shell_commands[] = {
    ...
    { "volts", "read board voltage", read_supply_voltage },
#endif
#ifdef MODULE_RIOTBOOT
