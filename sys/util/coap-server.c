...
void coap_start_server(char *addr_str, uint16_t port)
{
    ...
    snprintf(thread_name, sizeof(thread_name), "coap/dtls svr %" PRIu16, port);
...
static kernel_pid_t _coap_server_pid = KERNEL_PID_UNDEF;

char _coap_server_stack[THREAD_STACKSIZE_MAIN +
THREAD_EXTRA_STACKSIZE_PRINTF];
...
void *_coap_server_wrapper(void *arg)
{
...
{
    /* Only one instance of the server */
    if (_coap_server_pid != KERNEL_PID_UNDEF) {
        puts("Error: server already running");
        return;
    }

    static char thread_name[23];
    snprintf(thread_name, sizeof(thread_name), "coap server %" PRIu16, port);
    _coap_server_pid = thread_create(_coap_server_stack,
                                     sizeof(_coap_server_stack),
                                     THREAD_PRIORITY_MAIN - 1,
                                     THREAD_CREATE_STACKTEST,
                                     _coap_server_wrapper, (void *)&port, thread_name);

    /* Uncommon but better be sure */
    if (_coap_server_pid == EINVAL) {
        puts("ERROR: Thread invalid");
        _coap_server_pid = KERNEL_PID_UNDEF;
        return;
    }

    if (_coap_server_pid == EOVERFLOW) {
        puts("ERROR: Thread overflow!");
        _coap_server_pid = KERNEL_PID_UNDEF;
        return;
    }

    LOG_INFO("[coap server] started listening on port %" PRIu16 "\n", port);
...
void coap_stop_server(void)
{
    ...
    /* send the stop message to thread AND wait for (any) answer */
    msg_send_receive(&m, &m, _coap_server_pid);
