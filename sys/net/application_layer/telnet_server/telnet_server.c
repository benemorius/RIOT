...
#define TCP_SEND_TIMEOUT_US     (US_PER_SEC * 60)
...
static char _stdout_pipe_buf[1024];
...
static void *_shell_thread(void *arg)
{
    ...
    shell_run_once(util_all_shell_commands, line_buf, SHELL_DEFAULT_BUFSIZE);
...
#define KEEPALIVE_INTERVAL_MS (MS_PER_SEC * 60)
...
static int _recv(telnet_server_t *srv, char *c)
{
    ...
    while (1) {
        if (_is_sending) {
            xtimer_usleep(US_PER_MS * 100);
            continue;
        }
        ret = gnrc_tcp_recv(&srv->tcb, (void *)c, 1, US_PER_MS * 20);
        if (ret < 0) {
            switch (ret) {
                case -ETIMEDOUT:
...
static void *_server_thread(void *arg)
{
    ...
    while (1) {
        ...
        ret = gnrc_tcp_open_passive(&srv->tcb, &local);
        if (ret) {
