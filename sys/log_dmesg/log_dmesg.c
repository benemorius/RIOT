...
static char dmesg_buf_data[1024 * 14];
...
//     strftime(time, sizeof(time), "%Y-%m-%d %H:%M:%S", &tm);
    ...
    char uptime[12] = "";
#if 0 /* include uptime in log prefix */
    snprintf(uptime, sizeof(uptime), "[%2" PRIu32 ".%06" PRIu32 "]",
                                                            now.seconds,
                                                            now.microseconds);
#endif

    char file_line_function[128] = "";
#if 1 /* include file/line/function in log prefix */
    snprintf(file_line_function, sizeof(file_line_function), "%s:%u %s() \t",
                                                        file, line, function);
#endif

    snprintf(prefix, sizeof(prefix), "<%u><%u>%s[%s] %s",
             level, thread_getpid(), uptime, time, file_line_function);
