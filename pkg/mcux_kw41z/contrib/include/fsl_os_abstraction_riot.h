...
typedef thread_t      task_handler_t;

/*! @brief Type for a task stack.*/
typedef char*   task_stack_t;
...
typedef struct EventFreertos {
    ...
    void* eventHandler;    /*!< FreeRTOS OS event handler       */
    volatile uint32_t flags;
    volatile uint32_t threads;
} event_freertos;

/*! @brief Type for an event group object */
typedef event_freertos eventt_t;

/*! @brief Type for a message queue declaration and creation. */
typedef msg_t* msg_queue_t;

/*! @brief Type for a message queue handler */
typedef msg_t  msg_queue_handler_t;
